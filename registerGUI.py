...
def databaseEnter():
   ...
   if(name!="" and crime!="" and gen1!=""):
      ...
      cursor.execute('INSERT INTO People (Name,Gender,Father,Religion,Blood,Bodymark,Nationality,Crime) VALUES(?,?,?,?,?,?,?,?)',(name,gen1,father,religion,bl,body,nat,crime))
      conn.commit()
      x=getid()
...
def mfileopen():
   ...
   photo=ImageTk.PhotoImage(image)
   photo_label=Label(image=photo,width=500,height=600).place(x=740,y=100).pack()
